		...
		default:
			for i := 0; i < mtype.NumField(); i++ {
				...
				for _, opt := range opts {
					if opt.include && ((mtypef.Type.Kind() != reflect.Interface && !opt.omitempty) || !isZero(mvalf)) {
						val, err := e.valueToToml(mtypef.Type, mvalf)
						if err != nil {
							return nil, err
						}
						if tree, ok := val.(*Tree); ok && mtypef.Anonymous && !opt.nameFromTag && !e.promoteAnon {
							e.appendTree(tval, tree)
						} else {
							val = e.wrapTomlValue(val, tval)
							tval.SetPathWithOptions([]string{opt.name}, SetOptions{
								Comment:   opt.comment,
								Commented: opt.commented,
			...
			for i := 0; i < mtype.NumField(); i++ {
				...
				for _, opt := range opts {
					...
					if !found && opt.defaultValue != "" {
						...
						case reflect.Bool:
							val, err = strconv.ParseBool(opt.defaultValue)
						case reflect.Uint:
							val, err = strconv.ParseUint(opt.defaultValue, 10, 0)
						case reflect.Uint8:
							val, err = strconv.ParseUint(opt.defaultValue, 10, 8)
						case reflect.Uint16:
							val, err = strconv.ParseUint(opt.defaultValue, 10, 16)
						case reflect.Uint32:
							val, err = strconv.ParseUint(opt.defaultValue, 10, 32)
						case reflect.Uint64:
							val, err = strconv.ParseUint(opt.defaultValue, 10, 64)
						case reflect.Int:
							val, err = strconv.ParseInt(opt.defaultValue, 10, 0)
						case reflect.Int8:
							val, err = strconv.ParseInt(opt.defaultValue, 10, 8)
						case reflect.Int16:
							val, err = strconv.ParseInt(opt.defaultValue, 10, 16)
						case reflect.Int32:
							val, err = strconv.ParseInt(opt.defaultValue, 10, 32)
						case reflect.Int64:
							// Check if the provided number has a non-numeric extension.
							var hasExtension bool
							if len(opt.defaultValue) > 0 {
								lastChar := opt.defaultValue[len(opt.defaultValue)-1]
							...
							if hasExtension && mvalf.Type().String() == "time.Duration" {
								val, err = time.ParseDuration(opt.defaultValue)
							} else {
								val, err = strconv.ParseInt(opt.defaultValue, 10, 64)
							}
						case reflect.Float32:
							val, err = strconv.ParseFloat(opt.defaultValue, 32)
						case reflect.Float64:
							val, err = strconv.ParseFloat(opt.defaultValue, 64)
						default:
					...
					if !found && opt.defaultValue == "" && mtypef.Type.Kind() == reflect.Struct {
						tmpTval := tval
						if !mtypef.Anonymous {
							tmpTval = nil
						}
